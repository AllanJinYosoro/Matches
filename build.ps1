...
$icon = Join-Path $root 'assets\matches.ico'
$sources = Get-ChildItem -LiteralPath (Join-Path $root 'src') -Recurse -Filter '*.cs' | Sort-Object FullName | ForEach-Object FullName
& $csc /nologo /target:winexe /platform:x64 /optimize+ /codepage:65001 /win32icon:$icon /out:$exe /reference:System.dll /reference:System.Core.dll /reference:System.Drawing.dll /reference:System.Windows.Forms.dll /reference:$windowsBase /reference:$presentationCore /reference:$systemXaml $sources
